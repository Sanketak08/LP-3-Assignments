...
class MergeSort {
    ...
    public static void threadedSort(Integer[] array) {
        ...
        // else if some remainder is present, then assume we have (actual_threads-1)
        // available workers
        // and assign the remaining elements to be worked upon by the remaining 1 actual
        // thread.
        boolean exact = length % MAX_THREADS == 0;
        int maxlim = exact ? length / MAX_THREADS : length / (MAX_THREADS - 1);
        // if workload is less and no more than 1 thread is required for work, then
        // assign all to 1 thread
        ...
        for (Thread t : threads) {
            try {
                // This implementation of merge requires, all chunks worked by threads to be
                // sorted first.
        ...
        // System.out.println("Merging k-parts array, where m number of parts are
        // distinctly sorted by each Threads of available MAX_THREADS="+MAX_THREADS);
        /*
         ...
         * For MAXLIMIT = 2 (2 elements per thread where total threads = 4, in a total
         * of 4*2 = 8 elements)
        ...
        for (int i = 0; i < length; i += maxlim) {
            ...
            int remain = (length) - i;
            int end = remain < maxlim ? i + (remain - 1) : i + (maxlim - 1);
            // System.out.println("Begin: "+0 + " Mid: "+ mid+ " End: "+ end + " MAXLIM = "
            // + maxlim);
...
class App {
    ...
    private static final int size = random.nextInt(10000);
